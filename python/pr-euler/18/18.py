...
def read_triangle(filename, separator):
    tr = []
    f = open(filename, 'r')

    for line in f:
        tr.append(map(int, line.rstrip().split(separator)))

    return tr


def find_heaviest_way_in_triangle(tr):
    for i in range(0, len(tr) - 1)[::-1]:
...
if __name__ == '__main__':
    start = time()
    tr = read_triangle('input', ' ')
    print find_heaviest_way_in_triangle(tr)
    print time() - start
