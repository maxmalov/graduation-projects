...
HWND hWndTest; //for testing

void				StealBMP(HWND, LPCTSTR);
...
//void StealBMP(HWND hWndDst, HWND hWndSrc, LPCTSTR lpszFileName)
//{
//	RECT rect, rectSrc; 
//	GetWindowRect(hWndSrc, &rectSrc);
//	HDC hdcDst = GetDC(hWndDst);
//	HDC hDstMemDC = CreateCompatibleDC(hdcDst);
...
//	rect.bottom = yScreen;
//
//	HBITMAP hBmpDst = CreateCompatibleBitmap(hdcDst, rectSrc.right - rectSrc.left, - rectSrc.top + rectSrc.bottom );
//	SelectObject(hDstMemDC, hBmpDst);
//	FillRect(hDstMemDC, &rect, (HBRUSH)CreateSolidBrush(RGB(255,255,255)));
//
//	HDC hdcSrc = GetDC(hWndSrc);
//	BitBlt(hDstMemDC, 0,0, xScreen, yScreen, hdcSrc,0,0,SRCCOPY);
//	ReleaseDC(hWndSrc, hdcSrc);
//
//	SaveCurrentBmp(lpszFileName, hBmpDst); 
//
...
LRESULT CALLBACK ViewProc(HWND hWndView,UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	static HDC hMemDC;
	static HBITMAP hBmpOld, hBmpOff, hBmpCopy;
	static BITMAP bm;
...
LRESULT CALLBACK WndProc(HWND hWndParent, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		//	break;
		case IDM_STEALCOPY:
			StealBMP(hWndParent,hWndTest, "test.bmp");
			break;
		case IDM_EXIT:
